...
当前在“记一笔”/编辑交易页（`TransactionForm.vue`）中，点击分录行的账户输入框会弹出一个居中的模态框（`AccountPicker.vue`）。该模态框带有深色半透明遮罩，覆盖在表单上方并阻止用户与背景表单交互。
...
- 保存 `showSheet` 状态；关闭时重置 `AccountSelector` 的 `expandedStack`（行为可保留为：下次打开时重置到根层级）。
