export default {
  common: {
    loading: 'Loading...',
    add: 'Add',
    delete: 'Delete',
    cancel: 'Cancel',
    save: 'Save',
    confirm: 'Confirm',
    close: 'Close',
    create: 'Create',
    edit: 'Edit',
    name: 'Name',
    description: 'Description',
    all: 'All',
    none: 'None',
  },
  nav: {
    transaction: 'Transactions',
    assets: 'Reports',
    accounts: 'Accounts',
    calendar: 'Calendar',
    budget: 'Budget',
    savingPlan: 'Saving Plans',
    plan: 'Plans',
  },
  accounts: {
    createAccount: 'New Account',
    moveTypeChangeConfirm:
      'After moving, this account and its sub-accounts will become "{type}" type. Continue?',
    moveFailed: 'Failed to move: {message}',
    types: {
      Asset: 'Asset',
      Income: 'Income',
      Expense: 'Expense',
      Equity: 'Equity',
    },
  },
  assets: {
    netWorth: 'Net Worth',
    totalAssets: 'Total Assets',
    totalLiabilities: 'Total Liabilities',
    liabilities: 'Liabilities',
    assetAccounts: 'Assets',
    noData: 'No asset data yet',
    tabs: {
      balanceSheet: 'Balance Sheet',
      cashFlow: 'Cash Flow',
    },
    period: {
      weekly: 'Week',
      monthly: 'Month',
      yearly: 'Year',
    },
    trend: {
      empty: 'No trend data yet',
    },
    cashFlow: {
      periodRange: '{start} to {end}',
      empty: 'No cash flow data',
    },
    category: {
      income: 'Income',
      expense: 'Expense',
      empty: 'No data',
    },
  },
  calendar: {
    newTransaction: 'New Transaction',
    confirmDelete: 'Are you sure you want to delete this transaction?',
  },
  budget: {
    confirmDelete: 'Are you sure you want to delete this budget?',
    nameRequired: 'Please enter a budget name',
    limitRequired: 'Please add at least one limit',
    saveFailed: 'Failed to save: {message}',
    new: 'New Budget',
    empty: 'No budgets yet',
    editTitle: 'Edit Budget',
    newTitle: 'New Budget',
    nameLabel: 'Budget Name',
    namePlaceholder: 'Enter budget name',
    periodLabel: 'Period',
    period: {
      daily: 'Daily',
      weeklySun: 'Weekly (starting Sunday)',
      weeklyMon: 'Weekly (starting Monday)',
      monthly: 'Monthly',
      yearly: 'Yearly',
      once: 'One-off',
    },
    deadlineLabel: 'Deadline',
    deadlinePrefix: 'Due',
    limitsTitle: 'Limits',
    selectAccount: 'Select account',
    limitPlaceholder: 'Limit',
    addLimit: 'Add Limit',
    create: 'Create',
    expired: 'Expired',
    remainingLabel: 'Left',
    overspentLabel: 'Over',
    detail: {
      currentPeriod: 'Current period',
      limit: 'Limit',
      actual: 'Actual',
      remaining: 'Remaining',
    },
  },
  savingPlan: {
    confirmDelete: 'Are you sure you want to delete this saving plan?',
    nameRequired: 'Please enter a plan name',
    targetRequired: 'Target amount must be greater than 0',
    accountsRequired: 'Please select at least one account',
    duplicateAccount: 'This account is already selected',
    saveFailed: 'Failed to save: {message}',
    new: 'New Saving Plan',
    empty: 'No saving plans yet',
    editTitle: 'Edit Saving Plan',
    newTitle: 'New Saving Plan',
    nameLabel: 'Plan Name',
    namePlaceholder: 'Enter plan name',
    periodLabel: 'Period',
    period: {
      daily: 'Daily',
      weeklySun: 'Weekly (starting Sunday)',
      weeklyMon: 'Weekly (starting Monday)',
      monthly: 'Monthly',
      yearly: 'Yearly',
      once: 'One-time',
    },
    deadlineLabel: 'Deadline',
    deadlinePrefix: 'Due',
    targetLabel: 'Target Amount',
    targetPlaceholder: 'Enter target amount',
    accountsTitle: 'Accounts',
    selectAccount: 'Select account',
    addAccount: 'Add Account',
    create: 'Create',
    expired: 'Expired',
    metBadge: 'Met',
    gapBadge: 'Gap {amount}',
    detail: {
      bookTitle: 'Book Basis',
      allocationTitle: 'Allocation Basis',
      targetAmount: 'Target',
      currentBalance: 'Balance',
      gap: 'Gap',
      met: 'Met',
      metYes: 'Yes',
      metNo: 'No',
      allocated: 'Allocated',
      satisfaction: 'Satisfaction',
      balance: 'Balance',
      occupied: 'Occupied',
      accountAllocated: 'Allocated',
    },
  },
  transactions: {
    confirmDelete: 'Are you sure you want to delete this transaction?',
    new: 'New Transaction',
    monthLabel: '{month}/{year}',
    monthlyExpense: 'Monthly Expenses',
    monthlyIncome: 'Monthly Income',
    monthlyBalance: 'Balance This Month',
    loadingMore: 'Loading more...',
  },
  txFilter: {
    filter: 'Filter',
    filtered: 'Filtered',
    timeRange: 'Date Range',
    accounts: 'Accounts',
    addAccount: 'Pick an account to add',
    tags: 'Tags',
    channels: 'Channels',
    members: 'Members',
    keyword: 'Description Search',
    keywordPlaceholder: 'Search description...',
    reimbursable: 'Reimbursable only',
    reset: 'Reset',
    done: 'Done',
    preset: {
      thisMonth: 'This Month',
      lastMonth: 'Last Month',
      last3Months: 'Last 3 Months',
      thisYear: 'This Year',
      all: 'All',
    },
  },
  txCard: {
    transfer: 'Transfer',
    refundPrefix: 'Refund · ',
  },
  txList: {
    empty: 'No transactions yet',
    daySummary: 'In: ¥{income} Out: ¥{expense}',
    weekdays: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
  },
  calendarGrid: {
    monthLabel: '{month}/{year}',
    weekdays: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
  },
  drawer: {
    rootNote: 'Root account, not editable',
    billingDayLabel: 'Billing day (day N of each month)',
    repaymentDayLabel: 'Repayment day (day N of each month)',
    ownersLabel: 'Owners',
    closeAccount: 'Close account',
    reopenAccount: 'Reopen account',
    deleteAccount: 'Delete account',
    nameUpdated: 'Name updated',
    billingDayUpdated: 'Billing day updated',
    repaymentDayUpdated: 'Repayment day updated',
    ownersUpdated: 'Owners updated',
    accountClosed: 'Account closed',
    accountReopened: 'Account reopened',
    deleteConfirm: 'Are you sure you want to delete the account "{name}"?',
  },
  createDrawer: {
    title: 'New Account',
    nameLabel: 'Account name',
    namePlaceholder: 'Enter account name',
    parentLabel: 'Parent account',
    parentHint: 'Will be created as a child of "{name}"',
    creating: 'Creating...',
    confirmCreate: 'Create',
    nameRequired: 'Please enter an account name',
  },
  txForm: {
    createTitle: 'New Transaction',
    editTitle: 'Edit Transaction',
    saveFailed: 'Failed to save: {message}',
    dateTime: 'Date & Time',
    description: 'Description',
    optional: 'Optional',
    member: 'Member',
    memberPlaceholder: 'Select member',
    tags: 'Tags',
    tagPlaceholder: 'Select tag',
    channelPath: 'Channel Path',
    postings: 'Postings',
    account: 'Account',
    commodity: 'Currency',
    amount: 'Amount',
    reimbursable: 'Reimbursable',
    addPosting: '+ Add Posting',
  },
  picker: {
    selectPlaceholder: 'Select account',
  },
  pickerOverlay: {
    typeAsset: 'Assets',
    typeIncome: 'Income',
    typeExpense: 'Expenses',
    typeEquity: 'Equity',
    back: '← Back',
    selectTitle: 'Select Account',
    confirm: 'Confirm Selection',
  },
  channelInput: {
    channelNumber: 'Channel #{id}',
    selectChannel: 'Select channel',
  },
  header: {
    title: 'Accounting',
  },
  config: {
    title: 'Settings',
    tabs: {
      member: 'Members',
      channel: 'Channels',
      tag: 'Tags',
      language: 'Language',
      account: 'Account',
    },
    newNamePlaceholder: 'Enter a new name...',
    emptyMembers: 'No members yet — add one',
    emptyChannels: 'No channels yet — add one',
    emptyTags: 'No tags yet — add one',
    channelDescPlaceholder: 'Enter a description...',
    linkedAccount: 'Linked account',
    notLinked: 'Not linked',
    importRules: 'Import Rules',
    mappingCategoryPlaceholder: 'e.g. Expenses:Dining',
    mappingAccountPlaceholder: 'Select target account',
    importBill: 'Import Bill',
    importing: 'Importing…',
    importMemberLabel: 'Import as member',
    importSummary: 'Imported {imported}, skipped {skipped}',
    importFailed: 'Import failed: {message}',
    importErrorDetails: 'Skipped details',
    importErrorRow: 'Row {row}: {detail}',
  },
  language: {
    label: 'Display language',
    en: 'English',
    'zh-CN': '简体中文',
  },
  auth: {
    username: 'Username',
    password: 'Password',
    submit: 'Sign in',
    submitting: 'Signing in...',
    loginFailed: 'Sign-in failed',
    rateLimited: 'Too many attempts, please try again later',
    totpHint: 'This account has two-factor authentication enabled.',
    codeLabel: 'Verification code',
    codePlaceholder: '6-digit code or recovery code',
    verify: 'Verify',
    verifyFailed: 'Verification failed',
    backToPassword: 'Back to password sign-in',
    pendingExpired: 'Sign-in session expired, please enter your password again',
    logout: 'Sign out',
    totp: {
      title: 'Two-factor authentication (TOTP)',
      enabled: 'Enabled',
      start: 'Set up two-factor authentication',
      rebind: 'Rebind two-factor authentication',
      setupFailed: 'Setup failed',
      enableFailed: 'Enable failed',
      scanHint: 'Scan the QR code with your authenticator app, then enter the code to finish.',
      enable: 'Enable',
      recoveryHint: 'Recovery codes are shown only once — save them somewhere safe.',
      done: 'Done',
    },
  },
}
