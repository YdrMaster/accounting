...
> 若数据库尚未初始化，请先用 CLI 初始化：
> ```bash
> cargo run --bin accounting -- my.db initialize --lang zh-CN
> ```
...
```bash
cargo run --bin accounting-api -- --db my.db --static-dir accounting-web/dist
```
...
```bash
# CLI 录入
cargo run --bin accounting -- my.db tx add \
  --date 2024-06-01 \
  --description "午餐" \
