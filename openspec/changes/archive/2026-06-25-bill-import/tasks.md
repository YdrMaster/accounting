...
- [x] 1.5 定义 `BillPosting` 结构体：`account_path: String`、`commodity_symbol: String`、`amount: Decimal`、`is_reimbursable: bool`
- [x] 1.6 定义 `AdaptError` 错误类型：`RowError { row: usize, message: String }`、`FormatError(String)` 等 variant，实现 `std::error::Error` 和 `Display`
- [x] 1.7 定义适配器注册机制：`fn builtin_adapters() -> Vec<Box<dyn BillAdapter>>` 返回所有内置适配器列表；`fn find_adapter(name: &str, adapters: &[Box<dyn BillAdapter>]) -> Option<&dyn BillAdapter>`
- [x] 1.8 编写 unit test：构造 `Vec<u8>` 测试数据验证 `find_adapter` 按 name 查找适配器
...
- [x] 3.1 在 `accounting-sql/src/schema.rs` 的 `SEED_ACCOUNTS_ROOT_EN` / `SEED_ACCOUNTS_ROOT_ZH` 中添加 `Import` / `导入` 系统根账户（`is_system=1, parent_id=NULL`）
- [x] 3.2 在 `SEED_TAGS_EN` / `SEED_TAGS_ZH` 中添加 `pending` / `待处理` 系统 Tag（`is_system=1`，description 说明用途）
- [x] 3.3 更新 `test_seed_data` 测试中的计数断言以反映新增的 seed 数据
- [x] 3.4 验证 `SEED_CLOSURE` 递归 CTE 会自动覆盖新增的 Import 根账户（因为 `WHERE is_system = 1` 已包含）

## 4. AccountService 扩展 — ensure_cascading

- [x] 4.1 在 `accounting-service/src/account_service.rs` 中新增 `ensure_cascading` 方法：与 `create_cascading` 类似但跳过 `AccountType::from_str` 校验，支持在系统根账户（如 Import）下创建子账户
- [x] 4.2 `ensure_cascading` 签名：`pub async fn ensure_cascading(&self, path: &str) -> Result<AccountId, AccountingError>`
- [x] 4.3 实现逻辑：按 `:` 分割路径，逐级查找/创建账户（`is_system=false`），仅当路径首段已存在且为系统账户时跳过 AccountType 校验；首段不存在时报错
- [x] 4.4 编写 unit test：验证 `ensure_cascading("Import:支付宝:餐饮美食")` 能在 Import 系统根账户下创建三级子账户
- [x] 4.5 编写 unit test：验证重复调用 `ensure_cascading` 返回相同 AccountId（幂等）
...
- [x] 7.1 运行 `cargo fmt` 确保所有新代码格式化
- [x] 7.2 运行 `cargo clippy` 确保零警告
- [x] 7.3 运行 `cargo test` 确保所有测试通过（含新增测试和现有回归测试）
- [x] 7.4 验证 `accounting import --source alipay --member 1 --file <path>` 端到端工作正常