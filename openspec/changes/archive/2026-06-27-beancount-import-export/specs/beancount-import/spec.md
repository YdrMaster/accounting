...
系统 SHALL 从 beancount `open` 指令中解析账户，使用 `account_get_or_create_by_path` 创建账户。`account_type` metadata 为 `"Import"` 的账户 SHALL 从 `Equity:Import:xxx` 路径还原为 `Import:xxx`。`billing_day` 和 `repayment_day` metadata SHALL 同步更新。`close` 指令 SHALL 设置账户的 `closed_at` 日期。
...
- **WHEN** beancount 文件中包含 `open 权益:导入:支付宝` 及 metadata `account_type: "Import"`
- **THEN** 系统 SHALL 创建账户路径 `导入:支付宝`（从 Equity:Import 前缀还原为 Import）
...
系统 SHALL 从 beancount 交易指令中解析 Transaction 和 Posting，还原所有 metadata 承载的本系统独有概念。

#### Scenario: 导入普通交易
- **WHEN** beancount 文件中包含交易指令，narration 为 "盒马买菜"，包含两条分录
- **THEN** 系统 SHALL 创建 Transaction { description: "盒马买菜", kind: Normal } 及对应 Posting

#### Scenario: 导入含 kind metadata 的交易
- **WHEN** 交易指令包含 `kind: "refund"` metadata
- **THEN** 系统 SHALL 设置 Transaction.kind 为 Refund

#### Scenario: 导入含 member metadata 的交易
- **WHEN** 交易指令包含 `member: "张三"` metadata
- **THEN** 系统 SHALL 查找名为 "张三" 的 Member 并设置 Transaction.member_id

#### Scenario: 导入含 channel_path metadata 的交易
- **WHEN** 交易指令包含 `channel_path: '[{"position":0,"channel":"微信","reconciled":true}]'` metadata
- **THEN** 系统 SHALL 查找名为 "微信" 的 Channel 并创建对应 ChannelPath 记录

#### Scenario: 导入含标签的交易
- **WHEN** 交易指令包含 `#餐饮`
- **THEN** 系统 SHALL 查找或创建名为 "餐饮" 的 Tag 并关联到该交易

#### Scenario: 导入含 cost 的分录
- **WHEN** 分录行包含 `100 USD {720 CNY}`
- **THEN** 系统 SHALL 设置 Posting.amount=100, cost=Some(720), cost_commodity 指向 CNY

#### Scenario: 导入含 reimbursable metadata 的分录
- **WHEN** 分录 metadata 包含 `reimbursable: TRUE`
- **THEN** 系统 SHALL 设置 Posting.is_reimbursable 为 true
