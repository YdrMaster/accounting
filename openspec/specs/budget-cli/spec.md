...
CLI SHALL 提供 `budget show` 子命令，接受预算表名称参数和可选 `--date` 参数（默认当天），显示预算执行情况。
...
CLI SHALL 提供 `budget update` 子命令，接受预算表名称参数和可选的 `--name`、`--period`、`--commodity`、`--limit` 参数。提供 `--limit` 时替换所有限额。`--commodity` 使用币种符号。
