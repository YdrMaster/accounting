...
#### Scenario: 创建映射记录
- **WHEN** 为成员 1、渠道"支付宝"设置 `"Expenses:餐饮美食" → AccountId(42)`
- **THEN** `account_mappings` 表中插入一行 `(member_id=1, channel_id=1, category="Expenses:餐饮美食", account_id=42)`

#### Scenario: 同一 (成员, 渠道, category) 重复设置覆盖
- **WHEN** 对同一 `(member_id=1, channel_id=1, category="Expenses:餐饮美食")` 先后设置 account_id=42 和 account_id=50
...
`category` 字段 SHALL 使用 `"<role>:<原始分类>"` 格式。role 为 `"Assets"`（资产侧）、`"Income"`（收入侧）或 `"Expenses"`（支出侧/退款侧）。资产侧映射 key 格式为 `"Assets:<付款方式名>"`，收入侧映射 key 格式为 `"Income:<分类名>"`，支出侧及退款映射 key 格式为 `"Expenses:<分类名>"`。
...
#### Scenario: 设置映射成功
- **WHEN** 调用 `mapping_set(member_id=1, channel_id=1, category="Expenses:餐饮美食", account_path="Expenses:餐饮")`
- **THEN** 系统查找 "Expenses:餐饮" 的 AccountId，插入或更新映射记录

#### Scenario: 设置映射目标账户不存在
- **WHEN** 调用 `mapping_set(category="Expenses:餐饮美食", account_path="Expenses:不存在")`
...
#### Scenario: 删除映射
- **WHEN** 调用 `mapping_delete(member_id=1, channel_id=1, category="Expenses:餐饮美食")`
- **THEN** 删除对应的映射记录

#### Scenario: 删除不存在的映射
- **WHEN** 调用 `mapping_delete(member_id=1, channel_id=1, category="Expenses:不存在")`
- **THEN** 系统返回映射不存在的错误
...
#### Scenario: CLI 设置映射
- **WHEN** 用户执行 `accounting mapping set --member Alice --channel 支付宝 --category "Expenses:餐饮美食" --account "Expenses:餐饮"`
- **THEN** 系统创建映射并输出成功信息
...
#### Scenario: CLI 删除映射
- **WHEN** 用户执行 `accounting mapping delete --member Alice --channel 支付宝 --category "Expenses:餐饮美食"`
- **THEN** 系统删除映射并输出成功信息
